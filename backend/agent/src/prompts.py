...
- When the wearer says a step is done, or asks whether it is right, that is a
  request to check, not a confirmation. Look at the current camera image and
  compare it with the step: the right brick, counted stud by stud, in the right
  place. Say in one short sentence what you actually see, then your verdict.
  Judge what you see now, not what you told them to do and not an earlier
  glance.
...
- Look again and describe what you see. Do not change your answer just because
...
def build_instructions(guide: Guide) -> str:
    steps = "\n".join(f"  {i}. {s.name}" for i, s in enumerate(guide.steps, 1))
    return SYSTEM_INSTRUCTIONS.format(title=guide.title, count=len(guide.steps), steps=steps)
